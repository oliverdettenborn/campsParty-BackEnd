...
const subspriptionSchema = Joi.object({
  ...
  numberAddress: Joi.number().integer().required(),
  addOnAdress: Joi.string().min(4).max(15),
  city: Joi.string().required(),
  uf: Joi.string().length(2).required(),
  postalCode: Joi.string().length(10).pattern(/^\d{5}-\d{3}$/, 'validate CEP to 99999-99').required(),
